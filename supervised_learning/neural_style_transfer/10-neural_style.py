...
class NST:
    ...
    def __init__(self, style_image, content_image, alpha=1e4, beta=1,
        ...
        if var is not a non-negative number, raise a TypeError
        with the message var must be a non-negative number
    ...
    def gram_matrix(input_layer):
        """ Method gram matrix
        input_layer - an instance tf.Tensor/tf.Variable of shape (1, h, w, c)
        ...
        raise a TypeError with the message input_layer must be tensor rank 4
    ...
    def compute_grads(self, generated_image):
        ...
        J_var is the variational costo
        """
        """if not isinstance(generated_image, tf.Tensor) or\
                generated_image.shape is not self.content_image:
            raise TypeError("generated_image must be a tensor of shape {}".
        ...
        return(tape.gradient(J, generated_image), J, J_content, J_style, J_var)
    ...
    def generate_image(self, iterations=1000, step=None,
        ...
        print
        Cost at iteration {i}: {J_total}, content {J_content}, style {J_style}
        i is the iteration
        ...
        if step is not None and not an integer, raise TypeError with message
        step must be an integer
        if step is not None and not positive or less than iterations , raise
        ...
        if beta1 is not in the range [0, 1], raise a ValueError with message
        beta1 must be in the range [0, 1]
        if beta2 is not a float, raise a TypeError with the message
        beta2 must be a float
        if beta2 is not in the range [0, 1], raise a ValueError with message
